...
def test_storage_slots_calcution_correct(
    client, user, vintage_factory, storage_factory, storage_item_factory
):
    ...
    item_deleted = storage_item_factory(
        storage=storage, vintage=vintage, user=user, row=1, column=1, deleted=True
    )
    item = storage_item_factory(
        storage=storage, vintage=vintage, user=user, row=1, column=1
    )
    item = storage_item_factory(
        storage=storage, vintage=vintage, user=user, row=1, column=2
    )
